...
class PropertyController extends Controller
{
    ...
    public function actionView($id)
    {
     ...
     public function actionPropertyform($management_id, $owner_id, $id='')
    {
        if($id == ''){
          $model = new Property();  
        }
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            ...
            if (Yii::$app->request->isAjax)
            {
               return $dh->processResponse($this, $model, 'propertyform', 'success', 'Successfully Saved!', 'pjax-'.$keyword, $keyword.'-form-alert-'.$model->id);
               exit;               
            }
            ...
            if (Yii::$app->request->isAjax)
            {
                return $dh->processResponse($this, $model, 'propertyform', 'danger', 'Please fix the below errors!', 'pjax-'.$keyword, $keyword.'-form-alert-0');
               exit; 
                     
            }
            else{
                return $this->render('propertyform', [
                    'model' => $model,
                ]);
            }
        }
